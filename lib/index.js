...
function cdf( x, options ) {
	...
	if ( isMatrixLike( x ) ) {
		...
		return cdf5( out, x, opts.d1, opts.d2 );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.d1, opts.d2 );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.d1, opts.d2, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.d1, opts.d2, opts.accessor );
		}
		return cdf2( out, x, opts.d1, opts.d2 );
	}
	return NaN;
} // end FUNCTION cdf()
